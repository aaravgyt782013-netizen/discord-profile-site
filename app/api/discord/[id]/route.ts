...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!data && lantern) {
    ...
    data = {
      ...
      discord_user: { id, username: m.username, global_name: m.global_name, avatar: m.avatar || null, banner: null, accent_color: null, public_flags: flags, avatar_decoration_data: null },
      activities: Array.isArray(lantern.activities) ? lantern.activities.map((a: any) => ({ name: a.name, type: 0, details: a.details ?? null, state: a.state ?? null, assets: a.assets ? { large_image: a.assets.large_image?.image_url } : undefined })) : [],
    };
  }
  ...
  data.discord_user = { ...data.discord_user, id, avatar: avatarHash, banner: bannerHash, public_flags: flags, avatar_decoration_data: decorationAsset ? { asset: decorationAsset } : null };
