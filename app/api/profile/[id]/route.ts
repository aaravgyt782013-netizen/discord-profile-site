...
async function get(url:string){
 try{
  const r=await fetch(url,{cache:"no-store",headers:{Accept:"application/json","User-Agent":"discord-profile-site/3.0"}});
...
export async function GET(_req:Request,ctx:{params:Promise<{id:string}>}){
 ...
 if(!data&&lantern){
  ...
  data={
   ...
   discord_user:{id,username:m.username,global_name:m.global_name,avatar:m.avatar||null,public_flags:m.flags?.bitfield,accent_color:m.accent_color||null,avatar_decoration_data:m.avatar_decoration_data||null},
 ...
 const avatarUrl=c.avatarUrl||c.avatar_url||c.display_avatar_url||c.avatar?.link||l.avatar?.link||null;
 const bannerUrl=c.bannerUrl||c.banner_url||c.banner?.link||l.banner?.link||null;
 ...
 return NextResponse.json({
  ...
  data,
  profile:{
   avatarUrl:resolvedAvatar,bannerUrl:resolvedBanner,avatarDecorationUrl:decorationUrl,
