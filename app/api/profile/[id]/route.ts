...
const BADGES:[number,string,string][]=[
 ...
 [256,"HypeSquad Balance","BALANCE"],[512,"Early Supporter","EARLY"],[16384,"Bug Hunter Gold","BUG+"],
 [131072,"Early Verified Bot Developer","DEV"],[262144,"Certified Moderator","MOD"],[4194304,"Active Developer","ACTIVE"]
];

async function get(url:string){
 try{
  const r=await fetch(url,{cache:"no-store",headers:{Accept:"application/json","User-Agent":"discord-profile-site/5.0"}});
  if(!r.ok)return null;
  return await r.json();
 }catch{return null}
}
function stringValue(...v:unknown[]){return v.find(x=>typeof x==="string"&&x.trim()) as string|undefined}
function objectId(v:any){
 if(typeof v==="string")return v;
 return stringValue(v?.id,v?.asset,v?.asset_id,v?.hash);
}
...
function normalizeBadges(value:unknown,flags:unknown):Badge[]{
 ...
 const normalized=value.map((b:any,i)=>{
  ...
  const label=stringValue(b?.label,b?.name,b?.description,b?.text);
...
function decorationUrl(v:any){
 const direct=imageUrl(v);
 if(direct)return direct;
 const asset=objectId(v);
 return asset?`https://cdn.discordapp.com/avatar-decoration-presets/${asset}.png?size=512`:null;
}
function avatarFromHash(id:string,hash:string|null){
 if(!hash)return null;
 return `https://cdn.discordapp.com/avatars/${id}/${hash}.${hash.startsWith("a_")?"gif":"webp"}?size=1024`;
}
function bannerFromHash(id:string,hash:string|null){
 if(!hash)return null;
 return `https://cdn.discordapp.com/banners/${id}/${hash}.${hash.startsWith("a_")?"gif":"webp"}?size=2048`;
}
...
export async function GET(req:Request,ctx:{params:Promise<{id:string}>}){
 ...
 if(!data&&lantern){
  ...
  data={
   discord_status:STATUSES.has(lantern.status)?lantern.status:"offline",
